          ...
          >
            <div className="md:w-1/3">
              <div className="max-w-xs">
                {dienst.slug === "" ? (
                  <span className=" cursor-pointer w-fit max-w-xs font-medium text-3xl border-b leading-relaxed hover:bg-linear-to-t hover:from-50% hover:to-50% hover:from-tbsgreen hover:border-b-0">
                    {dienst.title}
                  </span>
                ) : (
                  <Link
            ...
            <div className="md:w-2/3 flex flex-col gap-4">
              {Array.isArray(dienst.description) ? (
                ...
                <div className="m-0">{dienst.description}</div>
              )}
            </div>
          </div>
        ))}
