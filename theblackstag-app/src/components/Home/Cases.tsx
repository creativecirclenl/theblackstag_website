...
export default function Cases() {
  // Start with a stable ordering on server/client initial render.
  // Shuffle only after mount to avoid hydration mismatches caused by Math.random().
  const [items, setItems] = useState(initialSuccesVerhalen);

  useEffect(() => {
    // Fisher-Yates shuffle
    const shuffled = [...initialSuccesVerhalen];
  ...
  return (
    <section className="my-48">
      <div className="container md:flex justify-between items-end">
        <div>
          <h3 className="text-tbsblue text-2xl uppercase">Cases</h3>
